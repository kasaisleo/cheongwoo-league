...
const FILTER_LABEL_ACTIVE: Record<ActiveFilter, string> = { all: "전체", active: "활성", inactive: "비활성" };
...
/**
 * 실제 데이터 감사(2026-07) 결과 is_active=false인데 deleted_at=null인 row가 존재한다
 * (반대 조합은 없었음). 두 값이 불일치해도 항상 "비활성" 쪽으로 안전하게 처리한다 —
 * is_active만 보고 활성으로 잘못 표시하지 않기 위함.
 */
function isEffectivelyActive(member: AdminMemberRow): boolean {
  return member.is_active && !member.deleted_at;
}
  ...
  const filtered = useMemo(() => {
    ...
    let rows = members.filter((m) => {
      const matchesQuery = !q ||
        ...
        (m.phone ?? "").includes(q);
      const matchesActive = activeFilter === "all" || (activeFilter === "active" ? isEffectivelyActive(m) : !isEffectivelyActive(m));
...
function MemberRow({ member, isLast }: { member: AdminMemberRow; isLast: boolean }) {
  ...
  const isActive = isEffectivelyActive(member);
