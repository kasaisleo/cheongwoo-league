...
const ADMIN_SKIN_VARS: Record<string, Record<string, string>> = {
  default: {
    ...
    // ── Form Control 시맨틱 토큰 (A-1) — 기존 --admin-* 값을 그대로 파생 ──
    "--control-bg":             "var(--admin-surface)",
    "--control-bg-disabled":    "rgba(255,255,255,0.06)",
    "--control-text":           "var(--admin-text)",
    "--control-placeholder":    "var(--admin-muted)",
    "--control-border":         "var(--admin-border)",
    "--control-border-hover":   "var(--admin-border-strong)",
    "--control-border-focus":   "var(--admin-accent)",
    "--control-focus-ring":     "var(--admin-focus-ring, rgba(212,255,61,0.45))",
    "--control-selected-bg":    "var(--admin-accent)",
    ...
    // ── Action(Button) 시맨틱 토큰 (A-2-2) — 기존 --admin-* 값을 그대로 파생 ──
    "--action-primary-bg":       "var(--admin-accent)",
    "--action-primary-text":     "var(--admin-action-text)",
    "--action-primary-hover":    "color-mix(in srgb, var(--admin-accent) 85%, white)",
    "--action-secondary-bg":     "var(--admin-surface-raised)",
    "--action-secondary-text":   "var(--admin-text)",
    "--action-secondary-border": "var(--admin-border-strong)",
    "--action-ghost-text":       "var(--admin-text)",
    "--action-danger-bg":        "var(--admin-alert)",
    "--action-danger-text":      "var(--admin-action-text)",
    "--action-focus-ring":       "var(--admin-focus-ring)",
  },
  cheongwoo: {
    "--admin-bg":             "#111827",
    "--admin-bar-bg":         "#080e1a",
    ...
    "--control-border-hover":   "var(--admin-border-strong)",
    "--control-border-focus":   "var(--admin-accent)",
    "--control-focus-ring":     "var(--admin-focus-ring)",
    "--control-selected-bg":    "var(--admin-accent)",
    "--control-selected-text":  "var(--admin-bg)",
    "--control-muted-bg":       "rgba(255,255,255,0.04)",
    "--control-danger-border":  "var(--admin-alert)",
  ...
  namaste: {
    ...
    "--shell-sidebar-active-bg":   "var(--admin-accent-soft)",
    "--shell-sidebar-active-text": "var(--admin-accent)",
    // ── Form Control 시맨틱 토큰 (A-1) — deep plum-black / ivory / muted purple / lavender
    "--control-bg":             "var(--admin-bg)",
    "--control-bg-disabled":    "var(--admin-surface)",
    "--control-text":           "var(--admin-text)",
    "--control-placeholder":    "var(--admin-muted)",
    "--control-border":         "var(--admin-border)",
    "--control-border-hover":   "var(--admin-border-strong)",
